...
public:
    ...
    bool DFS(std::string text, int state, Path &a,int &init,int &end,int i, std::vector<std::vector<bool>> &visited, std::vector<int> &id_begins, std::vector<int> &id_ends, std::vector<bool> &id_group);
    void print();
    NFA();
    NFA(enum RuleType type, std::string by, std::string to);
    NFA(std::vector<int> my_set);
